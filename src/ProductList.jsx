...
const ProductList = ({ onHomeClick }) => {
  ...
  // Local state to track which plants were added (for button text/change)
  ...
  // Helper to check if item is in cart (either via local state or redux)
  const isAdded = (name) => {
    return addedToCart[name] === true || cartItems.some(item => item.name === name);
  };

  const totalQuantity = cartItems.reduce((sum, item) => sum + (item.quantity || 1), 0);

  // ────────────────────────────────────────────────
  //  Styles (you should eventually move most to CSS)
  // ────────────────────────────────────────────────
  const navbarStyle = {
  ...
  // Note: handlePlantsClick was referencing undefined setShowPlants → removed or commented
  // If you want a real "Plants" navigation later, consider using react-router
            ...
            <a href="#" onClick={handleCartClick} style={linkStyle}>
              <h1 className="cart">
                <svg 
                  ...
                  width="68"
                >
                  <circle cx="80" cy="216" r="12" fill="none" stroke="white" strokeWidth="2"/>
                  <circle cx="184" cy="216" r="12" fill="none" stroke="white" strokeWidth="2"/>
                  <path 
